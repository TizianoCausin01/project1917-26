function Project1917_dRSA_PLOT_allsubs_fixdep(cfg)
...
indirMEG234 = sprintf('%s%sresults%sdRSA%s%s_%dHz_%dstim_%dsec_%diterations_%dMNN_fixdep',rootdir,filesep,filesep,filesep,simstring,cfg.fsNew,90,cfg.stimlen,cfg.iterations,cfg.MNN);
...
for itest = 1:length(cfg.models2test)
    ...
    for iROI = 1:length(cfg.ROI)
        ...
        for isub = 1:length(cfg.subjects)
            ...
            if isub < 4% only free viewing
                ...
                if isub == 1 && itest == 1% fix dependent pixelwise model, need to average over runs
                    
                    dRSAreps = zeros(3,tRange*2+1);
                    for irep = 1:3
                        
                        fn = sprintf('%s%cSUB%02d_%s_%s_rep%d', indirMEG234, filesep, cfg.subjects(isub), cfg.ROInames{iROI}, cfg.modelnames{itest},irep);
                    ...
                    dRSAreal(isub,iROI,itest,1,:) = squeeze(mean(dRSAreps));
    ...
    for isim = 1:length(cfg.models2sim)
        ...
        if isim == 1 && itest == 1% fix dependent pixelwise model, need to average over runs
            
            dRSAreps = zeros(3,tRange*2+1);
            for irep = 1:3
                
                tempdir = '\\cimec-storage5.unitn.it\MORWUR\Projects\INGMAR\Project1917\results\dRSA\sim_corr_50Hz_90stim_10sec_50iterations_fixdep';
                fn = sprintf('%s%cSIM_%s_TEST_%s_SUB%02d_rep%d', tempdir, filesep, cfg.modelnames{isim}, cfg.modelnames{itest}, 2, irep);
                load(fn,'dRSA');
            ...
            dRSAsim(itest,isim,:) = squeeze(mean(dRSAreps));
