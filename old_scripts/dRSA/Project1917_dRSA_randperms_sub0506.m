function Project1917_dRSA_randperms_sub0506(cfg,isub,imod,iroi,iperm)
...
outdir = sprintf('%s%sresults%sdRSA%s%s_%dHz_%dstim_%dsec_%diterations_%dMNN_randperms_strictICA',rootdir,filesep,filesep,filesep,simstring,cfg.fsNew,cfg.nstim,cfg.stimlen,cfg.iterations,cfg.MNN);
if ~exist(outdir,'dir')
    mkdir(outdir);
end

fn2save = sprintf('%s%cSUB%02d_%s_%s_perm%04d', outdir, filesep, isub, cfg.ROInames{iroi}, cfg.modelnames{imod}, iperm);
...
data_final = data_final{1};% only free view
...
subperms = 10;% 10 permutations within single job
dRSA = zeros(subperms,length(latencytime));
for isubperm = 1:subperms
    
    %% create random permutation indices of pseudo-stimulus labels for within-subject statistics
    % first set random number generator again differently for each permutation
    % also make sure this random permutation is the same across all iterations in our subsampling loop because we still want to imitate what happens for
    % the real data, except the pseudo-stimulus indices are randomized
    rng(isub*10^7+iroi*10^6+iperm*10^5+second(datetime('now'))*10^4);
    randpermID = randperm((cfg.nstim*cfg.nstim-cfg.nstim)/2);
    ...
    for iter = 1:cfg.iterations
        ...
        % randomly permute the neural RDM
        neuralRDMvec = neuralRDMvec(randpermID,:);
    ...
    dRSA(isubperm,:) = squeeze(mean(dRSAlatency));
    
end% end subpermutation loop

% average over sub permutations
