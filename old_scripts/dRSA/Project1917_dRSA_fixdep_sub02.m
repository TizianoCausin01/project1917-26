function Project1917_dRSA_fixdep_sub02(cfg,isub,imod,irep,iroi)
...
indirMEG = fullfile(rootdir,'data',['SUB0' num2str(isub)],'preprocessing');
...
outdir = sprintf('%s%sresults%sdRSA%s%s_%dHz_%dstim_%dsec_%diterations_%dMNN_fixdep',rootdir,filesep,filesep,filesep,simstring,cfg.fsNew,cfg.nstim,cfg.stimlen,cfg.iterations,cfg.MNN);
if ~exist(outdir,'dir')
    mkdir(outdir);
end

fn2save = sprintf('%s%cSUB%02d_%s_%s_rep%d', outdir, filesep, isub, cfg.ROInames{iroi}, cfg.modelnames{imod}, irep);

%% load MEG and model data
% MEG data
fn2load = sprintf('%s%spreproc-data-final-MNN%d-%s-%dhz-sub%d_perrun',indirMEG,filesep,cfg.MNN,cfg.ROInames{iroi},cfg.fsNew,isub);
