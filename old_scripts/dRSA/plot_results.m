...
mods = ["SC"] %, "dg_map"]
imod_tit = ["spatial coherence" ] %, "Deep-Gaze II"]
sm = {}
roi_counter = 0
for iroi = rois
    ...
    for imod = mods
        counter = counter+1;
        sm{roi_counter} = plot_resultss(results_dir, imod,imod_tit(counter), iroi, parms, upper_y_lim)
    end
end
function storeMod = plot_resultss(results_directory,imod,imod_tit, iroi,parms, upper_y_lim)
% inputs:
% - results_directory
% - imod
