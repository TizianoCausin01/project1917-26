function Project1917_dRSA_avgordtest(cfg,isub,imod)
...
outdir = sprintf('%s%sresults%sdRSA%s%s_%dHz_%dstim_%dsec_%diterations_%dMNN_avgordtest',rootdir,filesep,filesep,filesep,simstring,cfg.fsNew,cfg.nstim,cfg.stimlen,cfg.iterations,cfg.MNN);
if ~exist(outdir,'dir')
    mkdir(outdir);
end
...
dRSAiter = zeros(cfg.iterations,framenum,framenum,'single');
for iter = 1:cfg.iterations
...
%     modelRDMvec = modelRDMvec - repmat(nanmean(modelRDMvec),size(modelRDMvec,1),1);
    ...
    if cfg.similarity == 0% simple correlation
        ...
        dRSAiter(iter,:,:) = corr(modelRDMvec,neuralRDMvec);
...
for iModelTime = 1:length(pseudotime)
    ...
    slice = squeeze(dRSA(iModelTime,timeidx));
...
dRSA = squeeze(mean(dRSAstack,1,'omitnan'));
